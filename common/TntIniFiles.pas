...
{    Copyright (c) 2005 Stanley Xu (http://getgosurf.com/)                    }
...
{$R-,T-,H+,X+}
{$DEFINE ENABLE_WIDEINIFILE_WIN9X}  // Allows experimental WideIniFile support on Windows9x. 
...
{$IFDEF ENABLE_WIDEINIFILE_WIN9X}
  { TTntMemIniFile_V2 - is designed to avoid synchronation-problems. It compares the last
    modified timestamp and reloads the content before reading or writing when necessary. }
  TTntMemIniFile_V2 = class(TTntMemIniFile)
  private
    FLastModifiedTime: Integer;
    procedure ResynchronizeSrcFile;
  public
    function ReadString(const Section, Ident, Default: WideString): WideString; override;
    procedure WriteString(const Section, Ident, Value: WideString); override;
    procedure ReadSection(const Section: WideString; Strings: TTntStrings); override;
...
{$IFDEF MSWINDOWS}  
  ...
  private
  {$IFDEF ENABLE_WIDEINIFILE_WIN9X}
    FAnsiIniFile: TTntMemIniFile_V2;  // For compatibility with Windows 95/98/Me
  {$ELSE}
...
uses
  RTLConsts, TntWideStrUtils, TntSystem
{$IFDEF MSWINDOWS}
  , Windows
{$ENDIF};
...
function GetFileTimeStamp(const FileName: WideString): Integer;
var
...
procedure TTntMemIniFile_V2.ResynchronizeSrcFile;
var
  ActFileTimeStamp: Integer;
begin
  ActFileTimeStamp := GetFileTimeStamp(FileName);
  if ActFileTimeStamp <> FLastModifiedTime then
  begin
    LoadValues;
    FLastModifiedTime := ActFileTimeStamp;
  end;
end;

function TTntMemIniFile_V2.ReadString(const Section, Ident, Default: WideString): WideString;
begin
  ResynchronizeSrcFile;
  Result := inherited ReadString(Section, Ident, Default);
end;

procedure TTntMemIniFile_V2.WriteString(const Section, Ident, Value: WideString);
begin
  ResynchronizeSrcFile;
  inherited WriteString(Section, Ident, Value);
  // Flush changes to disk
  TTntMemIniFile(Self).UpdateFile;
end;

procedure TTntMemIniFile_V2.ReadSection(const Section: WideString; Strings: TTntStrings);
begin
  ResynchronizeSrcFile;
  inherited ReadSection(Section, Strings);
end;

procedure TTntMemIniFile_V2.ReadSections(Strings: TTntStrings);
begin
  ResynchronizeSrcFile;
  inherited ReadSections(Strings);
end;

procedure TTntMemIniFile_V2.ReadSectionValues(const Section: WideString; Strings: TTntStrings);
begin
  ResynchronizeSrcFile;
  inherited ReadSectionValues(Section, Strings);
end;

procedure TTntMemIniFile_V2.DeleteKey(const Section, Ident: WideString);
begin
  ResynchronizeSrcFile;
...
procedure TTntMemIniFile_V2.EraseSection(const Section: WideString);
begin
  ResynchronizeSrcFile;
...
procedure TTntMemIniFile_V2.UpdateFile;
begin
  // Note: Because every changes have been flushed to disk immediately, so that we donnot have
  //       to anything here.
  //
  //inherited;   
end;
...
begin
  ...
  {$IFDEF ENABLE_WIDEINIFILE_WIN9X}
    FAnsiIniFile := TTntMemIniFile_V2.Create(FileName);
  {$ELSE}
...
begin
  ...
  if (not Win32PlatformIsUnicode) then FAnsiIniFile.Free;
  inherited Destroy;
end;
...
begin
  ...
  begin
    { Windows 95/98/Me }
  {$IFDEF ENABLE_WIDEINIFILE_WIN9X}
    FAnsiIniFile.ReadSections(Strings);
  {$ELSE}
...
begin
  ...
  begin
    { Windows 95/98/Me }
  {$IFDEF ENABLE_WIDEINIFILE_WIN9X}
    FAnsiIniFile.ReadSection(Section, Strings);
  {$ELSE}
...
begin
  ...
  begin
    { Windows 95/98/Me }
  {$IFDEF ENABLE_WIDEINIFILE_WIN9X}
    FAnsiIniFile.ReadSectionValues(Section, Strings);
  {$ELSE}
